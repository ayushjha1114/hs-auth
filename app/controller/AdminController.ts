...
import {
	customerTicketCreationEmail,
	engineerTicketCreationEmail,
} from "../helper/email";
...
class AdminController {
	static async validateAdmin(req, res) {
		...
		try {
			...
			if (userData !== "null") {
				...
				const flag = helper.comparePassword(
					decryptedPassword,
					userData.password
				);
				...
				if (flag) {
					...
					jwt.sign(
						helper.buildUserToken(buildTokenPayload),
					...
					return res
						.status(401)
						.json(Template.userdoesNotExist(ErrorMessage.INVALID_CREDS));
				}
				// }
			} else {
				...
				return res
					.status(404)
					.json(Template.userdoesNotExist(ErrorMessage.USER_NOT_FOUND));
			}
		} catch (error) {
			...
			return res
				.status(500)
				.json(
					Template.error(ErrorMessage.TECHNICAL_ERROR, ErrorMessage.LOGIN_ERROR)
				);
		}
	}
		...
		try {
			...
			if (userData === "null" || Object.keys(userData).length === 0) {
				...
				if (insertedData && insertedData !== "null") {
					return res.json(
						Template.successMessage(SuccessMessage.USER_INSERTED)
					);
				} else {
					return res
						.status(400)
						.json(Template.errorMessage(ErrorMessage.REGISTER_ERROR_INSERT));
				}
			} else {
	...
	static async getUserListBySearch(req: Request, res: Response) {
		try {
			...
			if (response && response.length > 0) {
				...
				return res.json(
					Template.success({ rows: modifiedList }, SuccessMessage.USER_LIST)
				);
			}
			return res.json(Template.errorMessage(ErrorMessage.USER_LIST_ERROR));
		} catch (error) {
		...
		try {
			...
			if (results && results.affectedRows && results.changedRows) {
				...
				return res.json(
					Template.success(
						{ rows: results },
						SuccessMessage.USER_DETAIL_UPDATED
					)
				);
			}
			return res.json(
				Template.errorMessage(ErrorMessage.USER_DETAIL_UPDATE_ERROR)
			);
		} catch (error) {
			logger.error(`error getUserList ${error}`);
			return res.json(Template.error());
	...
	static async getUserById(req: Request, res: Response) {
		try {
			...
			if (user && Object.keys(user).length > 0) {
				...
				user.password = "";
				return res.json(
					Template.success({ userDetail: user }, SuccessMessage.USER_BY_ID)
				);
			}
	...
	static async getServiceById(req: Request, res: Response) {
		try {
			...
			if (serviceDetail && Object.keys(serviceDetail).length > 0) {
				...
				return res.json(
					Template.success({ serviceDetail }, SuccessMessage.SERVICE_BY_ID)
				);
			}
	...
	static async createBrand(req: Request, res: Response) {
		try {
			...
			if (response && Object.keys(response).length > 0) {
				...
				return res.json(
					Template.success({ rows: response }, SuccessMessage.BRAND_CREATED)
				);
			}
			return res.json(Template.errorMessage(ErrorMessage.USER_BY_ID_ERROR));
		} catch (error) {
	...
	static async getBrandList(req: Request, res: Response) {
		try {
			...
			if (response && Object.keys(response).length > 0) {
				...
				return res.json(
					Template.success(
						{ rows: response.rows, totalCount: response.totalCount },
						SuccessMessage.BRAND_LIST
					)
				);
			}
			return res.json(Template.errorMessage(ErrorMessage.USER_BY_ID_ERROR));
	...
	static async updateBrand(req: Request, res: Response) {
		try {
			...
			if (response && response[0] === 1) {
				...
				return res.json(
					Template.success({ rows: response }, SuccessMessage.BRAND_UPDATE)
				);
			}
			return res.json(Template.errorMessage(ErrorMessage.USER_BY_ID_ERROR));
		} catch (error) {
	...
	static async createService(req: Request, res: Response) {
		try {
			...
			if (response && Object.keys(response).length > 0) {
				...
				return res.json(
					Template.success({ rows: response }, SuccessMessage.SERVICE_CREATED)
				);
			}
			return res.json(Template.errorMessage(ErrorMessage.USER_BY_ID_ERROR));
		} catch (error) {
	...
	static async updateService(req: Request, res: Response) {
		try {
			...
			if (response && response[0] === 1) {
				...
				return res.json(
					Template.success({ rows: response }, SuccessMessage.SERVICE_UPDATE)
				);
			}
			return res.json(Template.errorMessage(ErrorMessage.USER_BY_ID_ERROR));
		} catch (error) {
					...
					Template.success(
						{
							...
							paymentDetailList: response.paymentDetailList,
						},
						SuccessMessage.TICKET_LIST
					)
				);
			}
			return res.json(Template.errorMessage(ErrorMessage.USER_BY_ID_ERROR));
		...
		try {
			...
			if (response && Object.keys(response).length > 0) {
				...
				return res.json(
					Template.success(
						{ rows: paymentData },
						SuccessMessage.PAYMENT_DETAIL_SAVED
					)
				);
			}
			return res.json(Template.errorMessage(ErrorMessage.USER_BY_ID_ERROR));
	...
	static async getAllPaymentDetail(req: Request, res: Response) {
		try {
			...
			if (paymentData && Object.keys(paymentData).length > 0) {
				...
				return res.json(
					Template.success({ rows: paymentData }, SuccessMessage.TICKET_LIST)
				);
			}
			return res.json(Template.errorMessage(ErrorMessage.USER_BY_ID_ERROR));
		} catch (error) {
	...
	static async getTicketById(req: Request, res: Response) {
		try {
			...
			if (ticket && Object.keys(ticket).length > 0) {
				...
				return res.json(
					Template.success({ ticket }, SuccessMessage.SERVICE_BY_ID)
				);
			}
			return res.json(Template.errorMessage(ErrorMessage.TECHNICAL_ERROR));
		} catch (error) {
	...
	static async getTicketDetailById(req: Request, res: Response) {
		try {
			logger.info("function getTicketDetailById ");
			const { id } = req.params;
			...
			logger.error(`error getTicketDetailById ${error}`);
			return res.json(Template.error());
		}
	}

	static async saveFollowUp(req: Request, res: Response) {
		try {
			...
			if (response && Object.keys(response).length > 0) {
				...
				return res.json(Template.success(SuccessMessage.FOLLOWUP_SAVED));
			}
			return res.json(Template.errorMessage(ErrorMessage.USER_BY_ID_ERROR));
		} catch (error) {
			logger.error(`error saveFollowUp ${error}`);
			return res.json(Template.error());
		}
	}
}
